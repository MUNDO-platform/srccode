...
def dbstore_create(context, data_dict): 
    '''Adds a new DB store.
    ...
    **Results:**
    ...
    if not 'db_url' in data_dict:
        raise p.toolkit.ValidationError({'db_url': ['db_url required']}) 
    
    if 'package_id' in data_dict: 
        db_url_response = requests.get(data_dict.get('db_url'))

        if db_url_response is None:
            raise p.toolkit.ValidationError({'db_url': ['db_url is wrong']})

        if db_url_response.json() is None:
            raise p.toolkit.ValidationError({'db_url_response': ['db_url_response is None']})   
        
        resource_name = data_dict.get('name').lower()
        resource_url = data_dict.get('db_url')
        resource_webstore_url = data_dict.get('db_url')
        resource_dict = {'package_id': data_dict.get('package_id'),'name':resource_name,'url':resource_url,'webstore_url':resource_webstore_url,'resource_type':'dbstore','format':'JSON'}
        ...
        resource_url = data_dict.get('db_url')
        resource_webstore_url = data_dict.get('db_url')
        resource_dict = {'package_id': package_id,'name':resource_name,'url':resource_url,'webstore_url':resource_webstore_url,'resource_type':'dbstore','format':'JSON'}
        resource_create_response = toolkit.get_action('resource_create')(context, resource_dict)
        log.info(resource_create_response)
...
@logic.side_effect_free
def dbstore_delete(context, data_dict):
    '''Deletes a dbstore.

    :param name: dbstore id.
    :type name: string
...
@logic.side_effect_free
def dbstore_show(context, data_dict):
...
@logic.side_effect_free
def dbstore_get(context, data_dict):
