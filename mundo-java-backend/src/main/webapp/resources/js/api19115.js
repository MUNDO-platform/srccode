...
{    
    ...
    {
        $http.get('/cbr/mundo-java-backend/gui/user/api19115')
    ...
    $scope.saveQueue = function()
    {
        if ($scope.validate())
        {
            $http.post("/cbr/mundo-java-backend/gui/admin/api19115", $scope.queue)
                .success(function() 
                {     
                    $scope.getQueueList(); 
    ...
    $scope.removeQueue = function(id)
    {
        if (confirm("Are you sure?")) {
	        ...
	        $http.delete('/cbr/mundo-java-backend/gui/admin/api19115/'+id)
	            .success(function() 
	            {     
	                $scope.getQueueList(); 
